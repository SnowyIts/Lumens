...
public final class Predictions extends Module {
   ...
   private final BooleanSetting showTime = new BooleanSetting("Время", true);
   private final BooleanSetting landingBox = new BooleanSetting("Маркер приземления", true);
   private final BooleanSetting landingRing = new BooleanSetting("Кольцо", true);

   private Predictions() {
   }

   private ColorRGBA lineColor(int i) {
      switch (this.colorMode.get()) {
         ...
         default: return Lumens.getInstance().getThemeManager().getClientColor(i);
      }
   }

   @EventTarget
   public void onDraw(EventRender2D e) {
      if (!this.showTime.isEnabled()) return;
   ...
   public void onWorldRender(EventRender3D e) {
      ...
      this.getProjectiles().forEach((entity) -> {
         ...
         for(int i = 0; i < 300; ++i) {
            ...
            Render3DUtil.drawLine(prevPos, pos, this.lineColor(i).mulAlpha(MathHelper.clamp((float)i / 25.0F, 0.0F, 1.0F)).getRGB(), this.lineWidth.getCurrent(), false);
   ...
   private void BreakingBad(Entity entity, Vec3d pos, int ticks) {
      ...
      if (this.landingBox.isEnabled()) {
         net.minecraft.util.math.Box box = new net.minecraft.util.math.Box(pos.x - 0.15D, pos.y - 0.02D, pos.z - 0.15D, pos.x + 0.15D, pos.y + 0.15D, pos.z + 0.15D);
         Render3DUtil.drawBox(box, this.lineColor(ticks).getRGB(), this.lineWidth.getCurrent(), true, true, false);
      }
      if (this.landingRing.isEnabled()) {
         int segments = 24;
         float r = 0.4F;
         for (int i = 0; i < segments; i++) {
            double a1 = (Math.PI * 2.0D * i) / segments;
            double a2 = (Math.PI * 2.0D * (i + 1)) / segments;
            Vec3d p1 = new Vec3d(pos.x + Math.cos(a1) * r, pos.y + 0.02D, pos.z + Math.sin(a1) * r);
            Vec3d p2 = new Vec3d(pos.x + Math.cos(a2) * r, pos.y + 0.02D, pos.z + Math.sin(a2) * r);
            Render3DUtil.drawLine(p1, p2, this.lineColor(i * 8).getRGB(), this.lineWidth.getCurrent(), false);
         }
      }
   }
