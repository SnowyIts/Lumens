      ...
      if (e.isKeyDown(this.friendBind.getKeyCode())) {
         ...
         if (var4 instanceof EntityHitResult) {
            ...
            if (var5 instanceof PlayerEntity) {
               ...
               if (Lumens.getInstance().getFriendManager().isFriend(player.getGameProfile().getName())) {
                  Lumens.getInstance().getFriendManager().removeFriend(player.getGameProfile().getName());
               } else {
                  Lumens.getInstance().getFriendManager().add(player.getGameProfile().getName());
               }
            }
         }
      }
