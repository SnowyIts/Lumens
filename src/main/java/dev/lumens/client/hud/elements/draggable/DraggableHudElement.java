package dev.lumens.client.hud.elements.draggable;
...
public abstract class DraggableHudElement implements IMinecraft {
   ...
   protected void renderYLine(CustomDrawContext ctx, DraggableHudElement.SheetCode nearest) {
      if (nearest.pos == -1.0F) {
         ...
         ctx.drawRoundedRect(0.0F, nearest.pos, (float)ctx.getScaledWindowWidth(), 1.0F, BorderRadius.ZERO, Lumens.getInstance().getThemeManager().getCurrentTheme().getColor().mulAlpha(1.0F));
   ...
   public float getWidth() {
    ...
    public DraggableHudElement.Align getAlign() {
       return this.align;
    }
