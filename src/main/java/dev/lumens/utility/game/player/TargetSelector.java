package dev.lumens.utility.game.player;
...
public class TargetSelector implements IClient {
   ...
   private Stream<LivingEntity> createStreamFromEntities(Iterable<Entity> entities, float maxDistance, boolean ignoreWalls) {
      return StreamSupport.stream(entities.spliterator(), false)
         ...
         .filter((entity) -> {
            return !entity.getCommandTags().contains(Cosmetics.PET_TAG);
         })
         .filter((entity) -> {
   ...
   public static class EntityFilter {
      ...
      private boolean isValidEntityType(LivingEntity entity) {
         if (entity instanceof PlayerEntity) {
            PlayerEntity player = (PlayerEntity)entity;
            return Lumens.getInstance().getFriendManager().isFriend(player.getGameProfile().getName()) ? false : this.targetSettings.contains("Игроков");
