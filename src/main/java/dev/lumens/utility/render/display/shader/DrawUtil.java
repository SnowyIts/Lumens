package dev.lumens.utility.render.display.shader;
...
import dev.lumens.utility.interfaces.IWindow;
...
import dev.lumens.utility.render.display.base.CustomSprite;
...
public final class DrawUtil implements IWindow {
   ...
   public static void initializeShaders() {
      rectangleProgram = new GlProgram(Lumens.id("rectangle/data"), VertexFormats.POSITION_COLOR);
      squircleProgram = new GlProgram(Lumens.id("squircle/data"), VertexFormats.POSITION_COLOR);
      squircleTextureProgram = new GlProgram(Lumens.id("squircle_texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
      roundedTextureProgram = new GlProgram(Lumens.id("texture/data"), VertexFormats.POSITION_TEXTURE_COLOR);
      borderProgram = new GlProgram(Lumens.id("border/data"), VertexFormats.POSITION_COLOR);
      figmaBorderProgram = new GlProgram(Lumens.id("corner/data"), VertexFormats.POSITION_COLOR);
      loadingProgram = new GlProgram(Lumens.id("loading/data"), VertexFormats.POSITION_COLOR);
      gradientRectangleProgram = new GlProgram(Lumens.id("gradient_rectangle/data"), VertexFormats.POSITION_COLOR);
      blurProgram = new GlProgram(Lumens.id("blur/data"), VertexFormats.POSITION_COLOR);
      metaballsProgram = new GlProgram(Lumens.id("metanoise/data"), VertexFormats.POSITION_COLOR);
   }
   ...
   public static void drawShadow(MatrixStack matrices, float x, float y, float width, float height, float softness, BorderRadius borderRadius, ColorRGBA color) {
    ...
    public static void drawHudBg(MatrixStack matrices, float x, float y, float width, float height, float blurRadius, BorderRadius borderRadius, ColorRGBA color) {
       boolean blur = true;
       try {
          if (Interface.INSTANCE != null && Interface.INSTANCE.hudBlur != null) {
             blur = Interface.INSTANCE.hudBlur.isEnabled();
          }
       } catch (Throwable ignored) {
       }
       if (blur) {
          drawBlur(matrices, x, y, width, height, blurRadius, borderRadius, color);
       } else {
          drawRoundedRect(matrices, x, y, width, height, borderRadius, color);
       }
    }
   ...
   public static void drawGlow(MatrixStack matrixStack, float x, float y, float width, float height, int glowRadius) {
      Render2DUtil.drawGradientBlurredShadow(matrixStack, x, y, width, height, glowRadius, Gradient.of(Lumens.getInstance().getThemeManager().getCurrentTheme().getColor(), Lumens.getInstance().getThemeManager().getCurrentTheme().getColor(), Lumens.getInstance().getThemeManager().getCurrentTheme().getSecondColor(), Lumens.getInstance().getThemeManager().getCurrentTheme().getSecondColor()));
   }
