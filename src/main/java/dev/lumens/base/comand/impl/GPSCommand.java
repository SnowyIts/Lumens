...
import dev.lumens.base.waypoint.Waypoint;
...
public class GPSCommand extends CommandAbstract {
   ...
   public void execute(LiteralArgumentBuilder<CommandSource> builder) {
      ((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)builder.then(arg("X", IntegerArgumentType.integer()).then(arg("Z", IntegerArgumentType.integer()).executes((context) -> {
         ...
         Lumens.getInstance().getWaypointManager().set(waypoint);
         ...
         Lumens.getInstance().getWaypointManager().setPlayerWaypoint(waypoint);
         MessageUtil.displayInfo("GPS создан и указывает на XZ: %s, %s".formatted(new Object[]{x, z}));
         return 1;
      })))))).then(literal("player").then(literal("remove").executes((context) -> {
         if (!Lumens.getInstance().getWaypointManager().isEmptyPlayerWaypoint()) {
            Lumens.getInstance().getWaypointManager().clearPlayerWaypoint();
         ...
         if (!Lumens.getInstance().getWaypointManager().isEmpty()) {
            Lumens.getInstance().getWaypointManager().clear();
